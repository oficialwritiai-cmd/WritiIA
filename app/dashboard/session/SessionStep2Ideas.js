...
export default function SessionStep2Ideas() {
    ...
    async function saveIdeasToLibrary(ideas) {
        ...
        const rows = ideas.map(idea => ({
            ...
            platform:         'Reels',
            goal:             'engagement',
            titulo:           idea.titulo || 'Idea sin título',
            script_full_text: [
                idea.titulo   ? `💡 IDEA: ${idea.titulo}` : '',
                idea.hook     ? `⚡ Hook: ${idea.hook}` : '',
                idea.descripcion ? `📝 ${idea.descripcion}` : '',
                idea.pilar    ? `🏷️ Pilar: ${idea.pilar}` : '',
                idea.cta      ? `📢 CTA: ${idea.cta}` : '',
            ].filter(Boolean).join('\n'),
            content: {
                titulo_angulo: idea.titulo,
                hook:          idea.hook || '',
        ...
        // Insert in batch, ignore duplicates
        const { error } = await supabase.from('library').insert(rows);
        if (error) console.error('[saveIdeasToLibrary]', error.message);
    }
    ...
    async function generateIdeas(appendMode = false) {
        ...
        try {
            ...
            // Save ideas to library as type 'idea'
            saveIdeasToLibrary(ideas).catch(e => {
                console.error('[Step2] library save:', e);
                setLibError('Las ideas se generaron pero no se pudieron guardar en Biblioteca. Intenta de nuevo.');
            });
        } catch (e) {
    ...
    // Support both API response format (titulo/hook/pilar) and DB format (idea_title/idea_description/content_type)
    const byPilar = generatedIdeas.reduce((acc, idea, idx) => {
        const key = idea.pilar || idea.content_type || 'General';
        if (!acc[key]) acc[key] = [];
        acc[key].push({
            ...idea,
            titulo:      idea.titulo      || idea.idea_title        || 'Idea sin título',
            hook:        idea.hook        || idea.idea_description   || '',
            pilar:       idea.pilar       || idea.content_type       || 'General',
            tipo:        idea.tipo        || idea.content_type       || '',
            descripcion: idea.descripcion || idea.idea_description   || '',
            cta:         idea.cta         || '',
            _idx:        String(idea.id   || idx),
        });
        return acc;
    }, {});
    ...
    return (
        <div>
            ...
            {libError && (
                <div style={{ padding: '10px 14px', background: 'rgba(251,191,36,0.08)', border: '1px solid rgba(251,191,36,0.2)', borderRadius: '10px', color: '#fbbf24', fontSize: '0.82rem', marginBottom: '16px', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                    ⚠️ {libError}
                    <button onClick={()=>setLibError('')} style={{ background:'none', border:'none', color:'#fbbf24', cursor:'pointer', fontSize:'1rem', padding:'0 4px' }}>×</button>
                </div>
            )}
