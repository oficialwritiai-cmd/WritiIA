    ...
    for (let attempt = 0; attempt <= maxRetries; attempt++) {
        try {
            ...
            if (!response.ok) {
                ...
                if (errMsg.includes('model: claude-3-5-sonnet')) {
                    console.warn('[Anthropic] Sonnet model not allowed or found. Falling back to Haiku...');
...
function processResponse(data) {
    ...
    if (!parsed || (Array.isArray(parsed) && parsed.length === 0)) {
        ...
        parsed = parsed.map(item => ({
            titulo_angulo: item.titulo_angulo || item.idea_title || item.titulo || 'Idea de Contenido',
            idea_title: item.idea_title || item.titulo_angulo || item.titulo || 'Idea de Contenido',
            idea_description: item.idea_description || item.descripcion_idea || item.descripcion || 'Sin descripción disponible',
            ...item
        }));
    }
