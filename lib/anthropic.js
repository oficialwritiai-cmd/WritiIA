 ...
 */
async function callAnthropic({ apiKey, model, systemPrompt, userMessage, maxTokens = 4096, temperature = 0.7, maxRetries = 3 }) {
    ...
    // Debugging (masked)
    console.log(`[Anthropic-v2.2] [${new Date().toISOString()}] Model: ${model || 'claude-haiku-4-5-20251001'} | MaxTokens: ${maxTokens} | Key: ${cleanApiKey.substring(0, 15)}...`);
    ...
    for (let attempt = 0; attempt <= maxRetries; attempt++) {
        try {
            ...
            if (!response.ok) {
                ...
                if (isModelError && model !== 'claude-haiku-4-5-20251001') {
                ...
                // Intercept Anthropic billing errors — never show the raw English message
                const rawMsg = err.error?.message || err.message || '';
                const isBillingError =
                    err.error?.type === 'invalid_request_error' && err.error?.message?.includes('credit_balance_too_low') ||
                    rawMsg.toLowerCase().includes('credit balance is too low') ||
                    rawMsg.toLowerCase().includes('plans & billing') ||
                    rawMsg.toLowerCase().includes('credit_balance') ||
                    response.status === 402;

                if (isBillingError) {
                    throw new Error('El servicio de IA no está disponible en este momento. Por favor contacta con soporte.');
                }

                throw new Error(rawMsg || 'Error al conectar con Anthropic');
            }
...
/**
 * Generate content ideas using Claude Haiku.
 * Ideas/planes corren dentro de un maxDuration=60s en Vercel — Sonnet 4.6
 * generando 16-24 ideas estructuradas en JSON podia acercarse o superar ese
 * limite (causando que Vercel mate la funcion y devuelva una pagina no-JSON).
 * Haiku 4.5 es mucho mas rapido para este tipo de output estructurado y
 * mantiene la calidad suficiente para bancos de ideas. NOTE: nombre de
 * funcion mantenido por compatibilidad con los callers existentes.
 */
...
/**
 * Chat with full conversation history — for Nico assistant.
 * Passes the last N messages so Nico remembers context.
 */
export async function chatWithHistory({ apiKey, systemPrompt, messages, maxTokens = 2048 }) {
    if (typeof window !== 'undefined') throw new Error('Server-side only.');

    const cleanApiKey = (apiKey || '').toString().replace(/['"\s]/g, '').trim();
    if (!cleanApiKey || cleanApiKey === 'placeholder-anthropic-key') throw new Error('API Key no configurada.');
    ...
    const validMessages = firstUser >= 0 ? cleanMessages.slice(firstUser) : cleanMessages;
