...
/**
 * Generate content ideas using Claude Sonnet.
 * Uses higher max_tokens since idea banks produce larger JSON payloads.
 * NOTE: function name kept as "...WithHaiku" for backward compatibility with callers.
 */
export async function generateIdeasWithHaiku({ apiKey, systemPrompt, userMessage }) {
    return await callAnthropic({
        ...
        maxTokens: 4096,
        temperature: 0.8,
        // Retries cortos: esta función corre dentro de un maxDuration=60s en Vercel.
        // Con maxRetries=3 (backoff 2/4/8s) el peor caso podia acercarse al limite
        // y Vercel mataba la funcion devolviendo una pagina no-JSON. Con 1 retry
        // (backoff 2s) dejamos mucho mas margen para la generacion real.
        maxRetries: 1,
    });
}
